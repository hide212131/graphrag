...
from graphrag.index.verbs.text.replace.typing import Replacement
...
def my_none(
    ...
    **_kwargs: dict,
    ...
    # table: Dataframe の"chank"列の各テキストに、"path"の列の値を先頭に改行付きで挿入
    table["text"] = "Filepath: " + table["path"] + "\n" + table["text"]
    return TableContainer(table=table)


@verb(name="my_text_replace")
def my_text_replace(
    input: VerbInput,
    column: str,
    to: str,
